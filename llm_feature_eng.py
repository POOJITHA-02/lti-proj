        ...
        try:
            response = ollama.chat(
                ...
                options={
                    ...
                    "num_thread": max(1, os.cpu_count() - 1),  # use most cores per call, leave 1 free for the OS
                },
                keep_alive="30m",  # keep the model resident in RAM between calls — reloading it from
                                    # disk on every single request is a common, easy-to-miss cause of
                                    # calls that take far longer than the model's size should justify
            )
