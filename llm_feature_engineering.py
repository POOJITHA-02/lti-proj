...
BATCH_SIZE = 1           # llama3.2 doesn't reliably follow multi-item batch instructions —
                          # 1 keeps each prompt short and fast instead of wasting tokens on
                          # products it was going to ignore anyway. Raise this only if you
                          # switch to a model that actually handles batches (see README).
MAX_RETRIES = 3
