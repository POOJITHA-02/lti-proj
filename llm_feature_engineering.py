...
MODEL = "llama3.2:1b"    # smaller model = real speedup on CPU-only hardware
BATCH_SIZE = 1
MAX_RETRIES = 3
MAX_WORKERS = 1           # start here to rule out memory pressure between concurrent calls forcing
                          # the model to unload/reload — raise back to 2-3 only after confirming
                          # single-worker speed is what it should be
...
def get_untagged_products(conn, limit: int) -> list[tuple[int, str, int, float, float]]:
    return conn.execute(
        """SELECT p.product_id, p.product_name,
           ...
           ORDER BY COALESCE(ps.purchase_count, 0) DESC
           LIMIT ?""",
        (limit,),
    ).fetchall()
...
def extract_json_object(text: str) -> dict | list | None:
    """Parses JSON from model output that may include markdown fences or stray text,
    now that we're not forcing strict grammar-constrained format="json" decoding."""
    text = text.strip()
    try:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            response = ollama.chat(
                ...
                # NOTE: deliberately NOT using format="json" — grammar-constrained JSON
                # decoding is known to be dramatically slower per-token on some
                # Ollama/llama.cpp versions. The prompt already asks for JSON-only output;
                # extract_json_object() below handles minor deviations leniently instead.
                options={
                    "num_predict": 60,   # output is a tiny JSON object — no need to allow long generations
                    "num_ctx": 512,      # prompt is short — smaller context window processes faster
                    "temperature": 0.1,  # classification task, not creative — low temp is fine and often quicker
                },
                keep_alive="30m",  # keep the model resident in RAM between calls
            )
        ...
        if not parsed:
            print(f"  attempt {attempt}/{MAX_RETRIES} — couldn't extract valid JSON. Raw response:\n{content}")
            time.sleep(1)
            continue
